%Created By Michael Griffin, 9/4/13

%Creates input files loaded by exp_ustatetrace.
...
waitTime2 = .5; %duration of blank screen. Needed for constructing the sound lists at end.
buffer = 4; %How many tones to hear before list begins/how long it continues after words finish
numlists = 8;
rate = 1.7; % how fast sounds are presented.
nback = 1; %what level of nback task is being used.
percentcorrect = .35; %how many trials of the tone Nback task are correct
lrec = 30;
numobs = lrec/6;
...
presrates = [{'.15'} {'.3'} {'.6'} {'1.2'}; {'.6'} {'1.2'} {'2.4'} {'4.8'}];
...
for i = 900
    ...
    %for Cresp. Subject number should also determine what color is chosen).
    ...
    for n = 1:numlists
        ...
        for o = 1:lrec
            if strcmp(testlists{n,1}{o,5}, 'oldnew')
                ...
                if strcmp(testlists{n,1}{o,3}, '1') %color 1 requires old response
    ...
    %For the divided attention task, people must press space whenever the
    %same tone was heard N sounds back.

    for m = 1:4
        check = 0;
        proportioncorrect = 0; %check for proportion matching percent correct.

        trials = zeros(lsound(m),1);
        numcorrect = ceil(lsound(m)*percentcorrect);
        correcttrials = nback+randperm(lsound(m)-nback);
        correcttrials = correcttrials(1:numcorrect);
        trials(correcttrials) = 1;

        soundlists{m,1}{1,1} = randi(5);
        soundlists{m,1}{1,5} = 'incorrect';
        switch nback
            case 2
                soundlists{m,1}{2,1} = randi(5);
                soundlists{m,1}{2,5} = 'incorrect';
            case 3
                soundlists{m,1}{2,1} = randi(5);
                soundlists{m,1}{3,1} = randi(5);
                soundlists{m,1}{2,5} = 'incorrect';
                soundlists{m,1}{3,5} = 'incorrect';
        end


        for n = 1+nback:length(soundlists{m,1})
            if trials(n) %if a correct trial
                soundlists{m,1}{n,1} = soundlists{m,1}{n-nback,1};
                soundlists{m,1}{n,5} = 'correct';
            else
                soundlists{m,1}{n,1} = randi(5);
                while soundlists{m,1}{n,1} == soundlists{m,1}{n-nback,1} || ...
						soundlists{m,1}{n,1} == soundlists{m,1}{n-1,1};

                    soundlists{m,1}{n,1} = randi(5);
                end
                soundlists{m,1}{n,5} = 'incorrect';
            end
        end
    end
    ...
    for m = 1:8
        ...
        if strcmp(currentlist{1,2}, 'Full')
            ...
            for n = 1:numsounds
                ...
                soundtimes{n,5} = currentsoundlist{n,5};
            end
...
%             else
%
%                 if strcmp(practicetestlists{n,1}{o,3}, '1') %If color 1 requires old response
...
%     for n = 1:2
%         practicelists{n,1}(1:length(practicelists{n,1})/2,:) = ...
%		  practicelists{n,1}(randperm(length(practicelists{n,1})/2),:);
...
%
%
%     %%Create Practice Sound List & Stand Alone Sound List, A Lesson in
%     %%copy paste.
%
%     for k = 1:2
%         lpracsound = ceil(((str2double(presrate)+waitTime2)*lprac)/rate+2*buffer);
%         currentsoundlist = cell(lpracsound, 5);
%
%         check = 0;
%         proportioncorrect = 0; %check for proportion matching percent correct.
%
%         trials = zeros(lpracsound,1);
%         numcorrect = ceil(lpracsound*percentcorrect);
%         correcttrials = nback+randperm(lpracsound-nback);
%         correcttrials = correcttrials(1:numcorrect);
%         trials(correcttrials) = 1;
%
%
%         currentsoundlist{1,1} = randi(5);
%         currentsoundlist{1,5} = 'incorrect';
%         switch nback
%             case 2
%                 currentsoundlist{2,1} = randi(5);
%                 currentsoundlist{2,5} = 'incorrect';
%             case 3
%                 currentsoundlist{2,1} = randi(5);
%                 currentsoundlist{3,1} = randi(5);
%                 currentsoundlist{2,5} = 'incorrect';
%                 currentsoundlist{3,5} = 'incorrect';
%         end
%
%
%         for n = 1+nback:length(currentsoundlist)
%             if trials(n) %if a correct trial
%                 currentsoundlist{n,1} = currentsoundlist{n-nback,1};
%                 currentsoundlist{n,5} = 'correct';
%             else
%                 currentsoundlist{n,1} = randi(5);
%                 while currentsoundlist{n,1} == currentsoundlist{n-nback,1} || ...
%						currentsoundlist{n,1} == currentsoundlist{n-1,1};
%                     currentsoundlist{n,1} = randi(5);
%                 end
%                 currentsoundlist{n,5} = 'incorrect';
%             end
%         end
%
%         switch k
%             case 1
%                 practicesoundlist = currentsoundlist;
%             case 2
%                 standalonesounds = currentsoundlist;
%         end
%     end
%
%
...
%     standaloneschedule(:,5) = standalonesounds(:,5);
%
...
%                 soundtimes{n,4} = 'sound';
%                 soundtimes{n,5} = practicesoundlist{n,5};
%             end
