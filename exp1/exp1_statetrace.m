%Created by Michael Griffin, 12/12/12

%Runs experiment 1 of the state-trace recognition experiments.
%Divided attention task is to listen for 3 odd numbers in a row

%Requires:
%subject file in input/						(generated by input_statetrace.m)
...
testwaitTime = 1; %this is for the blank screen between test items.
promptTime = 1.2; %this is for the time that 'Color' or 'Old/New' appears before the word is presented at test.
InitializePsychSound;
audiofiles = {};
audiofreq = {};
%alltesttimes: contains m,n cells, all gotten from 'testtimes'.
%1st col is RT, 2nd col is rawTime, 3rd is resp, 4th is Cresp
alltesttimes = cell(length(testlists), 1);
...
audiochannel = PsychPortAudio('Open', [], 1, [], audiofreq{1,1}, numchannels);
...
pause(2);  %I'm sorry, I didn't know that WaitSecs was better when I made this!
Screen('TextSize', windowPtr, 25);
WriteLine(windowPtr, ['In this experiment, you will study a series of ', int2str(lrec), ...
	' words that you need to learn for an upcoming memory test.  Each word will appear ' ...
	'on the screen for a few seconds and then disappear.  Words will be presented in '...
	'one of two colors, and it is important to remember both the word and the color '...
	'it was presented in for the memory test.'] , WordColor, 100, 175, 75, 1.3);
WriteLine(windowPtr, ['Sometimes, you will also hear a series of numbers while you '...
	'are studying. Whenever you hear 3 odd numbers in a row, you''ll need to press '...
	'the Space Bar. It is important to pay about equal attention to both studying '...
	'the words and listening for 3 consecutive odd numbers.'], WordColor, 100, 175, 350, 1.3);
Screen('Flip', windowPtr, [], 1);
pause(1.5);
...
Screen('Flip', windowPtr);
pause(waitTime2);
WriteLine(windowPtr, ['To get you familiar with the number task, there will be a short '...
	'practice on just this task.'], WordColor, 200, 175, 275, 1.3);
...
%%%%START OF PRACTICE FOR DA TASK, GIVES FEEDBACK
...
%START OF PRACTICE
for n = 2 %To shorten exp, only run the second practice list (which has sounds accompanying it).
    if n == 1
        Screen('TextSize', windowPtr, 25);
        WriteLine(windowPtr, ['Now a short practice study phase will start. Remember,' ...
			'words will be presented in one of two colors, and it is important to ' ...
			'remember both the word and the color it was presented in for the memory test.'], ...
			WordColor, 100, 175, 100, 1.3);
        WriteCentered(windowPtr, ['Press any key when you are ready to begin the ' ...
			'practice study phase.' ], xPos, ScreenHeight-100, WordColor);
        Screen('Flip', windowPtr);
        wait = KbWait;
        Screen('Flip', windowPtr);
        pause(2);
        ...
        WriteLine(windowPtr, ['Now a practice study phase will start. In this one, '...
			'you will again be hearing a series of numbers while studying. ' ...
			'Once again, remember to press the Space Bar each time you hear ' ...
			'3 consecutive odd numbers.'], WordColor, 100, 175, 100, 1.3);
        WriteCentered(windowPtr, ['Press any key when you are ready to begin the ' ...
		'next study phase.'], xPos, ScreenHeight-100, WordColor);
        Screen('Flip', windowPtr);
    ...
    if n == 2
        ...
        for s = 1:length(indexes)
            ...
            if strcmp(practicespacetimes{indexes(s), 5}, '3 odds')
                if spaceresp
        ...
        if percentfa || percentmiss > 40
             WriteLine(windowPtr, ['Your accuracy for the numbers task was a bit low. '...
			 'If you find yourself focusing solely on the words, try to focus more on '...
			 'listening to the numbers. Remember, both tasks are equally important, and '...
			 'you should be attending about equally to both of them.'], WordColor, 100, 175, 300, 1.3);
        else
    ...
    Screen('TextSize', windowPtr, 25);
    WriteLine(windowPtr, ['Now a practice test phase will begin. You will be asked to '...
		'either say whether a word was one you had studied, or to say what color the '...
		'studied word was.  For words prompted with Color, you should press d if '...
		'they were ',  currentcol1, ' or j if they were ', currentcol2, '. For words '...
		'prompted with Old/New, you should press c if they were studied, and n '...
		'if they were new.'], WordColor, 100, 175, 100, 1.3);
    Screen('Flip', windowPtr, [], 1);
    pause(2);
    ...
    for r = 1:length(currenttlist)
        ...
        WriteCentered(windowPtr, currenttlist{r,1}, xPos, yPos, WordColor);
        Screen('Flip', windowPtr, [], 1);
...
Screen('TextSize', windowPtr, 25);
WriteLine(windowPtr, ['Now the actual experiment will begin. Sometimes, like the '...
	'practice, you will both study words and hear numbers presented, while sometimes '...
	'there will only be words to study. At the start of each study phase, you''ll be '...
	'told which will be coming up. If both words and numbers will be presented, '...
	'remember to pay about equal attention to both.'], WordColor, 100, 175, 100, 1.3);
...
for n = 1:length(studylists)
    ...
    if strcmp(currentlist{1,2}, 'Divided')
        Screen('TextSize', windowPtr, 25);
        WriteLine(windowPtr, ['In this study phase, you will also be hearing a series '...
			'of numbers while studying. Once again, remember to press the Space Bar '...
			'each time you hear 3 consecutive odd numbers.'], WordColor, 100, 175, 100, 1.3);
        ...
        Screen('TextSize', windowPtr, 25);
        WriteLine(windowPtr, ['In this study phase, you will only be studying words. '...
			'Once again, try to remember both the word and the color for the following '...
			'memory test.'], WordColor, 100, 175, 100, 1.3);
        WriteCentered(windowPtr, 'Press any key when you are ready to begin the study phase.' , xPos, ScreenHeight-100, WordColor);
        Screen('Flip', windowPtr);
        Screen('TextSize', windowPtr, 40);
        wait = KbWait;
    ...
    if strcmp(currentlist{1,2}, 'Divided')
        ...
        for s = 1:length(indexes)
            ...
            if strcmp(spacetimes{indexes(s), 5}, '3 odds')
                if spaceresp
        ...
        if percentfa || percentmiss > 40
            ...
            WriteLine(windowPtr, ['Your accuracy for this list was good. Once again, '...
			'remember to try and pay about equal attention to the digit task and '...
			'studying the words themselves.'], WordColor, 100, 175, 300, 1.3);
        end
    end
