%Created By Michael Griffin, 1/29/13

%Creates input files loaded by exp_nstatetrace.
...
for i = 900
    ...
    for n = 1:numlists
        ...
        for o = 1:lrec
            if strcmp(testlists{n,1}{o,5}, 'oldnew')
                ...
                if strcmp(testlists{n,1}{o,3}, '1') %color 1 requires old response (note that 'color 1' actually is made to switch colors in the experiment script).
    ...
    for n = 1:2
        ...
        if sum(ismember(firsthalf(:,5), 'Discrim')) == 2*sum(ismember(firsthalf(:,5), 'oldnew')) %checks for test type consistency.
            checkcount(n,2) = 1;
        end
...
%        o = 1;
%        check = 0;
%        while o <= 8 && check == 0                  %if one grabs from MRC database, will leave behind extra spaces and such. This should remove them.
...
%        end
%
%        lsect = round(lprac*(1-trialratio)/2); %The round is a continued stupidity that matlab requires.
...
%            end
%            if rem(p, 2) == 0
%                practicetestlists{n,1}{p,3} = '1'; %This doesn't actually make much sense. Words will be shown in black regardless.
%            else
...
%        practicelists{n,1}(1+length(practicelists{n,1})/2:length(practicelists{n,1}),:) = practicelists{n,1}(randperm(length(practicelists{n,1})/2)+length(practicelists{n,1})/2,:); %scramble second half.
