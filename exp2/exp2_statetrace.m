%Created by Michael Griffin, 1/31/13

%Runs experiment 2 of the state-trace recognition experiments.
%Divided attention task has changed from experiment 1. Now a tone 1-back task.

%Requires:
%subject file in input/						(generated by input_nstatetrace.m)
...
WriteLine(windowPtr, ['In this experiment, you will study a series of ', int2str(lrec),...
	...
	'appear on the screen for a few seconds and then disappear.  Words will be '...
	'presented in one of two colors, and it is important to remember both the word '...
	'and the color it was presented in for the memory test.'], WordColor, 100, 175, 75, 1.3);
WriteLine(windowPtr, ['Sometimes, you will also hear a series of tones while you '...
	'are studying. Whenever you hear a tone that matches the one just before it, '...
	'you''ll need to press the Space Bar. It is important to pay attention both to '...
	'studying the words and to listening for tone repetitions.'], WordColor, 100, 175, 350, 1.3);
Screen('Flip', windowPtr, [], 1);
pause(1.5);
...
labelcoords = textcoords;
labelcoords(2,:) = rectcoords(2,:)-25;
labelcoords(4,:) = rectcoords(4,:)-actbounds(4)-25;


%START OF PRACTICE
for n = 2 %Due to time constraints, currently only running the second practice list (which has sounds accompanying it).
    if n == 1
        Screen('TextSize', windowPtr, 25);
        WriteLine(windowPtr, 'Now a short practice study phase will start. Remember, words will be presented in one of two colors, and it is important to remember both the word and the color it was presented in for the memory test.', WordColor, 100, 175, 100, 1.3);
        ...
        Screen('TextSize', windowPtr, 25);
        WriteLine(windowPtr, 'Now a practice study phase will start. In this one, you will again be hearing a series of tones while studying. Once again, remember to press the Space Bar each time the current tone matches the one heard just before it.', WordColor, 100, 175, 100, 1.3);
    ...
    WriteLine(windowPtr, ['Now a practice test phase will begin. You will be asked '...
		'to either say whether a word was one you had studied, or to say what color '...
		'the studied word was.  For words prompted with Color, you should press d '...
		'if they were ',  currentcol1, ' or j if they were ', currentcol2, '. For '...
		'words prompted with Old/New, you should press c if they were studied, and '...
		'n if they were new.'], WordColor, 100, 175, 100, 1.3);
    Screen('Flip', windowPtr, [], 1);
    pause(5);
...
%%%%INSTRUCTIONS FOR FULL EXPERIMENT
Screen('TextSize', windowPtr, 25);
WriteLine(windowPtr, ['Now the actual experiment will begin. Sometimes, like the '...
	'practice, you will both study words and hear tones presented, while sometimes '...
	'there will only be words to study. At the start of each study phase, you''ll '...
	'be told which will be coming up. If both words and numbers will be '...
	'presented, remember to pay about equal attention to both.'], WordColor, 100, 175, 100, 1.3);
WriteLine(windowPtr, ['As before, try to remember both the word and the color '...
	'for the following memory test.'], WordColor, 100, 175, 400, 1.3);
WriteCentered(windowPtr, 'Press any key when you are ready to continue.' , xPos, ScreenHeight-100, WordColor);
Screen('Flip', windowPtr);
wait = KbWait;
pause(1);

%Beginning of non-practice trials
for n = 1:length(studylists)
    ...
    if strcmp(currentlist{1,2}, 'Divided')
        ...
        WriteLine(windowPtr, ['In this study phase, you will only be studying words. '...
			'Once again, try to remember both the word and the color for the '...
			'following memory test.'], WordColor, 100, 175, 100, 1.3);
        WriteCentered(windowPtr, 'Press any key when you are ready to begin the study phase.' , xPos, ScreenHeight-100, WordColor);
        Screen('Flip', windowPtr);
        Screen('TextSize', windowPtr, 40);
        wait = KbWait;
    ...
    if strcmp(currentlist{1,2}, 'Divided')
        ...
        if percentfa || percentmiss > 40
			...
			'listening to the tones. Remember, both tasks are equally important, and '...
